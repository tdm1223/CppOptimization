...
## 6.7 평평한 자료구조
- 자료 구조의 요소들이 인접한 저장 공간에 **함께 저장**되었다면 자료구조가 평평하다고 할 수 있다. (`array`, `vector`)
- 평평한 자료구조는 포인터로 연결된 자료구조보다 생성할 때 메모리 관리자를 호출하는 횟수가 적다.
- `std::array`, `std::vector`와 같은 평평한 자료구조는 `list`, `map`, `unordered_map`과 같은 노드 기반의 자료구조보다 메모리를 적게 차지한다.
- 평평한 자료구조를 사용하면 스마트 포인터와 스마트 포인터가 가리키는 **객체를 할당하는 런타임 비용**을 없앨 수 있다.