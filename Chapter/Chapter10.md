...
- 순서가 지정되지 않은 연관 컨테이너
- 삽입 시간 : 평균 `O(1)`, 최악의 경우 `O(n)`
- 색인 시간 : 평균 `O(1)`, 최악의 경우 `O(n)` (키를 사용할 경우)
- 반복자는 다시 해시하면 무효화되며 참조는 항목을 삭제하는 경우에만 무효화된다.
- 크기와 관계없이 용량이 증가하거나 감소할 수 있다.
- 순서가 지정되지 않은 맵은 생성에 비용이 많이 든다.
  - 맵에는 각 테이블 항목에 동적으로 할당된 노드와 동적으로 크기를 조정할 수 있는 버킷 배열을 포함한다.

#### 로드 팩터
- 순서가 지정되지 않은 맵에 있는 항목의 개수를 크기(`size`)라고 한다.
- 크기를 버킷의 개수로 나눈 값을 로드 팩터(`load factor`)라고 한다.
- 로드 팩터가 **1.0보다 크면** 일부 버킷에 여러 항목의 체인이 있어 키의 검색 성능이 저하된다.
  - 해시가 완전하지 않다.
- 로드 팩터가 **1.0보다 작으면** 순서가 지정되지 않은 맵의내부 배열에 공간을 소비하는 버킷 중 일부가 사용되지 않는다는 것을 의미한다.
- 로드팩터가 프로그램에서 지정한 최대 로드 팩터를 초과하게 되면 버킷 배열이 재할당 되고 모든 항목들이 새 배열의 버킷으로 다시 해시된다.
- 컨테이너의 크기가 **버킷 로드 팩터**를 초과하기 전에는 재할당 하지 않는다.

### 10.8.1 std::unordered_map에서 삽입/삭제하기
- 순서가 지정되지 않은 맵은 맵처럼 두 가지 삽입 매서드를 제공한다.
  - 삽입 힌트가 있는 메서드
  - 삽입 힌트가 없는 매서드
- 순서가 지정되지 않은 맵은 맵과 달리 삽입 힌트를 사용하지 않는다.
  - **호환성을 제공할 뿐**이며 아무 작동을 수행하지 않아도 성능이 약간 저하된다.
- **최대 로드 팩터**를 아주 높게 설정하면 재할당을 연기할 수 있다.

### 10.8.2 std::unordered_map에서 반복하기
```cpp
for (auto it = test_container.begin(); it != test_container.end(); ++it)
{
  sum += it->second;
...
```
- 순서가 지정되지 않은 맵은 **정렬할 수 없으며** 컨테이너의 이름에서 알 수 있듯이 반복할 때 항목을 알 수 없는 순서로 생성한다.
- 순서가 지정되지 않은 맵을 반복하는 데 걸리는 시간은 효율적이지만 벡터를 반복하는 시간보다 2.8배 오래 걸린다.

### 10.8.3 std::unordered_map에서 검색하기
- 순서가 지정되지 않은 맵이 존재하는 이유는 검색 때문이다.
- 맵보다 3배 빠른 성능을 보여준다.
- `std::lower_bound()`를 사용해 정렬된 벡터에서 검색하는 것보다 1.7배 빠르다.

## 10.9 다른 자료구조
#### boost::circular_buffer
- `std::deque`와 비슷한 면이 많지만 더 효율적이다.

#### Boost.Container
- 표준 라이브러리 컨테이너를 변형한 컨테이너들이 있다.
- 재할당하더라도 반복자를 무효화 하지 않는 벡터인 안정 벡터
- std::vector용 컨테이너 어댑터로 구현된 맵/멀티맵/셋/멀티셋
- 가변 길이를 가지며 최댓값이 고정되어 있는 정적 벡터
- 벡터에 요소가 몇 개만 있을 떄 최적화된 작동을 하는 벡터
...
- 그래프 순회를 위한 알고리즘과 자료구조

#### boost.heap
- 간단한 std::priority_queue 컨테이너 어댑터보다 더 좋은 성능을 가지며 작동에 미묘한 차이가 있는 우선순위 큐

#### Boost.Intrusive
- 침습성 컨테이너(링크를 명시적으로 포함하는 노드 타입에 의존하는 컨테이너)를 제공한다.

#### boost.lockfree
- 락프리 및 웨이트프리인 큐와 스택

#### Boost.MultiIndex
- 서로 다른 작동을 가진 여러 색인이 있는 컨테이너